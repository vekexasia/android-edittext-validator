...
/**
 * A validating {@link EditTextPreference} validation is performed when the OK or AlertDialog.BUTTON_POSITIVE button is
 * clicked. When invalid an error message is displayed and the EditTextPreference is not dismissed
 */
    ...
    extends EditTextPreference
{
	...
	{
		super( context, attrs );
		editTextValidator = new DefaultEditTextValidator( getEditText(), attrs, context );
	}
	...
	protected void showDialog( Bundle state )
	{
		...
		if ( super.getDialog() instanceof AlertDialog )
		{
			...
			// get originalBottomPadding to know when adjust the underlying layouts bottom padding (ie has room already
			...
			{
				// some exception thrown. Unable to do increase space for error message
			}
			...
			b.setOnClickListener( new ValidatingOnClickListener( originalBottomPadding, theDialog ) );
		}
	}
		...
		public void onClick( View view )
		{
			...
			if ( editTextValidator.testValidity() )
			{
				...
				if ( originalBottomPadding != Integer.MIN_VALUE )
				{
					...
					{
						parentLayout.setPadding( parentLayout.getPaddingLeft(), parentLayout.getPaddingTop(), parentLayout.getPaddingRight(),
